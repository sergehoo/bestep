...
class CreateOrganizationMemberSerializer(serializers.Serializer):
    ...
    send_invitation_if_exists = serializers.BooleanField(required=False, default=False)
    ...
    def create(self, validated_data):
        actor = self.context["request"].user
        return OrganizationMemberService.create_member(
            actor=actor,
            organization=validated_data["organization"],
            email=validated_data["email"],
            ...
            send_invitation_if_exists=validated_data.get("send_invitation_if_exists", False),
        )