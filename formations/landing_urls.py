...
urlpatterns = [
    # ── P1.4 — Catalogue public server-rendered (SEO + fallback no-JS) ──
    path("catalogue/", PublicCatalogPageView.as_view(), name="public_catalog"),

    # ── API + pages existantes ──
    ...
    path("business/categories/<slug:slug>/", CategoryProfessionalCourseDetailView.as_view(), name="business_category_detail"),
    path("business/manifestation-interet/", BusinessInterestRequestCreateView.as_view(), name="business_interest_request"),
    path("api/public/courses/<int:course_id>/", PublicCourseDetailView.as_view(), name="api_public_course_detail"),
    path("courses/<slug:slug>-<int:course_id>/", CourseDetailPageView.as_view(), name="course_public_page"),
